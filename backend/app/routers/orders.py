...
def import_orders(req: dict, db: Session = Depends(get_db)):
    ...
    for item in data:
        try:
            validated = schemas.OrderItem(**item)
    ...
    ids = crud.create_orders(db, valid)
...
def assign_orders(req: schemas.OrdersAssignPostRequest, db: Session = Depends(get_db)):
    ...
    active_assigned = [o for o in assigned if o.status == "assigned"]

    if active_assigned:
        # Идемпотентность
        return {"orders": [{"id": o.order_id} for o in active_assigned],
                "assign_time": assigned[0].assign_time.isoformat()}
    ...
    for o in available:
        if o.weight > capacity: continue
        if o.region not in courier.regions: continue
        if not any(utils.hours_overlap(cw, oh) for cw in courier.working_hours for oh in o.delivery_hours): continue
        to_assign.append(o)
